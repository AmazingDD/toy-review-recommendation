...
def load_data_and_labels(train_data, valid_data, user_review, item_review, user_rid, item_rid, stopwords):
    ...
    user_reviews = dill.load(f1)
    item_reviews = dill.load(f2)
    user_rids = dill.load(f3)
    item_rids = dill.load(f4)
...
if __name__ == '__main__':
    ...
    dill.dump(batches_train, output)
    output = open(os.path.join(TPS_DIR, 'music.test'), 'wb')
    dill.dump(batches_test, output)
