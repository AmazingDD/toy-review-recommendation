...
parser.add_argument('-rp', '--raw_data_path', type=str, help='path for loading raw data', default='./music.txt')
parser.add_argument('-rt', '--data_ratio', type=str, help='ratio of train:valid:test', default='8:1:1')
parser.add_argument('-pd', '--processed_data_dir', type=str, help='the directory for saving processed data if -rp not None. load processed data from this directory if -rp None', default='./train/')
parser.add_argument('-ep', '--embedding_path', type=str, help='the path for loading pre-trained embedding', default='./pretrain/music_vectors.bin')
...
parser.add_argument('-lr', '--learning_rate', type=float, help='learning rate, default=0.001', default=0.004)
parser.add_argument('-dk', '--dropout_keep', type=float, help='dropout ratio, default=0.8', default=0.6)
...
for it in range(1, args.max_iter + 1):
    ...
    train_mae = mean_abs_error(train_predict, max_r, min_r)
    print(get_now_time() + 'MAE on train data: {}'.format(train_mae))
    ...
    valid_mae = mean_abs_error(valid_predict, max_r, min_r)
    print(get_now_time() + 'MAE on valid data: {}'.format(valid_mae))
    ...
    test_mae = mean_abs_error(test_predict, max_r, min_r)
    print(get_now_time() + 'MAE on test data: {}'.format(test_mae))
    # early stop setting
